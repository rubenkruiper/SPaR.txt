...
from lib.modules.multi_head_positional_attention import RelativeGlobalAttention
...
@Model.register("attention_tagger")
class AdaptedCrfTagger(Model):
    ...
    def forward(
        ...
        **kwargs,  # to allow for a more general dataset reader that passes args we don't need
        ...
        # Relative global posattention
        attended_text = self._attention(encoded_text)
        encoded_text = torch.cat([encoded_text, attended_text], dim=-1)
