...
class SpanBasedF1Measure(Metric):
    ...
    def __init__(
        ...
        if label_encoding:
            if label_encoding not in ["DiscontiguousTest"]:
                raise ConfigurationError(
                    "Unknown label encoding - expected 'DiscontiguousTest'."
                )
    ...
    def __call__(
        ...
        for i in range(batch_size):
            ...
            elif self._label_encoding == "DiscontiguousTest":
                tags_to_spans_function = discontiguous_tags_to_spans
            else:
